...
const AdminProductList = () => {
  ...
  const handleDelete = async (id) => {
    if (globalThis.confirm("Yakin ingin menghapus produk ini?")) {
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/admin/add-product">
          <button className="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 transition-colors shadow-md">
            + Tambah Produk
          </button>
        </Link>
      </div>
                ...
                <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <td className="p-4 align-top">
                    <div className="flex items-start gap-3">
                      ...
                      />
                      <div>
                        <span className="font-medium text-gray-800 dark:text-gray-200 block">{product.title}</span>
                        <span className="text-xs text-gray-500 line-clamp-2 mt-1">{product.description}</span>
                      </div>
                    </div>
                  </td>
